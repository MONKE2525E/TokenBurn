...
Open Settings → Diagnostics and choose **Copy logs**. That copies the current session's redacted log tail plus a client-state snapshot (settings, per-provider status and errors, refresh status, catalog and cache state) to the clipboard so it can be pasted into an assistant. The bundle is already scrubbed; no credentials or provider bodies are included. The full redacted log lives at `%LOCALAPPDATA%\UsageMonitor\Logs\usage-monitor.log`.
